...
type SortType = "default" | "quota-asc" | "quota-desc" | "last-used";

// 计算账号的总配额百分比 (用于排序)
const getAccountTotalQuota = (account: Account): number => {
  if (!account.quota?.models || account.quota.models.length === 0) return 0;
  const sum = account.quota.models.reduce((acc, m) => acc + m.percentage, 0);
  return sum / account.quota.models.length; // 返回平均值
};
...
function Accounts() {
  ...
  const [sortBy, setSortBy] = useState<SortType>(() => {
    const saved = localStorage.getItem("accounts_sort_by");
    return (saved as SortType) || "default";
  });

  // Save sort preference
  useEffect(() => {
    localStorage.setItem("accounts_sort_by", sortBy);
  }, [sortBy]);
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (sortBy === "quota-desc") {
      result = [...result].sort(
        (a, b) => getAccountTotalQuota(b) - getAccountTotalQuota(a),
      );
    } else if (sortBy === "quota-asc") {
      result = [...result].sort(
        (a, b) => getAccountTotalQuota(a) - getAccountTotalQuota(b),
      );
    } else if (sortBy === "last-used") {
      result = [...result].sort((a, b) => b.last_used - a.last_used);
    }
    // 'default' 保持原有顺序 (拖拽排序)

    return result;
  }, [searchedAccounts, filter, sortBy]);
  ...
  return (
    <div className="h-full flex flex-col p-5 gap-4 max-w-7xl mx-auto w-full">
      ...
      <div className="flex-none flex items-center gap-2">
        ...
        {/* 排序选择器 */}
        <div className="flex items-center gap-1 bg-gray-100 dark:bg-base-200 p-1 rounded-lg shrink-0">
          <ArrowUpDown className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400 ml-1" />
          <select
            className="bg-transparent text-xs font-medium text-gray-700 dark:text-gray-300 border-none focus:outline-none focus:ring-0 cursor-pointer pr-6 py-1"
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as SortType)}
          >
            <option value="default">{t('accounts.sort.default', '默认顺序')}</option>
            <option value="quota-desc">{t('accounts.sort.quota_desc', '用量最多')}</option>
            <option value="quota-asc">{t('accounts.sort.quota_asc', '用量最少')}</option>
            <option value="last-used">{t('accounts.sort.last_used', '最近使用')}</option>
          </select>
        </div>
